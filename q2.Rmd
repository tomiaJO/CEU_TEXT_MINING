...
In the next step two things are combined: unnesting the text column to words, and in the same step removing the useless ones (stopwords). We also add a column, line number.
...
The above visualization of the joint-distribution of word frequencies between the two authors should not carry much surpirse either - many of the noted words above with similar frequencies.  
...
Let's get started by creating a set for both enhanced with sentiment scores from the AFFIN lexicon:  
...
tidy_vh_books %>%
  ...
  ggplot(aes(word, word_count * score, fill = score)) + 
    geom_col(show.legend = FALSE) + 
    labs(y = "Contribution to sentiment", x = NULL) + 
    coord_flip() + 
    facet_grid(~gutenberg_id)
```
